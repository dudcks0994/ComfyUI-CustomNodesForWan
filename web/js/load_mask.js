...
app.registerExtension({
    name: "MyCustomNodes.LoadMask",
    async nodeCreated(node) {
        if (node.comfyClass !== "LoadMaskNode") return;
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.success && data.folder_path) {
                            ...
                            console.log(`[LoadMask] 폴더 선택됨: ${data.folder_path}`);
                        } else if (data.error) {
                            console.warn(`[LoadMask] 폴더 선택 실패: ${data.error}`);
                        ...
                        console.warn(`[LoadMask] API 응답 오류: ${response.status}`);
                        showBrowserFallback();
                    }
                } catch (error) {
                    console.error("[LoadMask] Error selecting folder:", error);
                    showBrowserFallback();
                }
            }
        );
